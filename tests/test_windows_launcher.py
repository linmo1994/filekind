...
def test_prepare_windows_frozen_default_argv_injects_run(
    ...
    argv = ["filekind.exe"]
    assert filekind_main.prepare_windows_frozen_default_argv(argv) is False

    monkeypatch.setattr(sys, "frozen", True, raising=False)
    monkeypatch.setattr(sys, "platform", "win32")

    argv = ["C:\\dist\\filekind.exe"]
    assert filekind_main.prepare_windows_frozen_default_argv(argv) is True
    assert argv[1:] == [
        "run",
        "--apply",
        "--no-dry-run",
    ...
    assert filekind_main.prepare_windows_frozen_default_argv(["filekind.exe", "--help"]) is False
